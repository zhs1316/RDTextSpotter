...
def get_args_parser():
    ...
    parser.add_argument('--use_customer_dictionary', action='store_true')
    ...
    parser.add_argument('--num_workers', default=2, type=int) # 2
    ...
    parser.add_argument('--rotate_max_angle', type=int, default=45)
...
def main(args):
    ...
    if args.distributed:
        ...
        evaluator = TextEvaluator(task=args.task, lexicon_type=args.lexicon_type, use_lexicon=args.use_lexicon, use_customer_dictionary=args.use_customer_dictionary, dataset_name=args.val_dataset, distributed=True)
    else:
        sampler_train = torch.utils.data.RandomSampler(dataset_train)
        sampler_val = torch.utils.data.SequentialSampler(dataset_val)
        evaluator = TextEvaluator(task=args.task, lexicon_type=args.lexicon_type, use_lexicon=args.use_lexicon, use_customer_dictionary=args.use_customer_dictionary, dataset_name=args.val_dataset, distributed=False)
    ...
    if args.eval:
        ...
        return evaluate(model, criterion, postprocessors,
                                              data_loader_val, base_ds, device, args.output_dir, wo_class_error=wo_class_error, args=args, TextEvaluator=evaluator)
