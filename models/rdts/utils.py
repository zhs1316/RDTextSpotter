...
def sigmoid_focal_giou_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2, giou_weights = None):
    """
    Loss used in RetinaNet for dense detection: https://arxiv.org/abs/1708.02002.
    Args:
        inputs: A float tensor of arbitrary shape.
    ...
    if giou_weights is not None:
        weights = giou_weights + 0.75 * prob.pow(2) * (1 - targets)
        loss = weights * loss
