...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
                 ...
                 # sampling
                 update_sampling_feature=True,
                 update_sampling_layers=5,
                 update_weight=0.5
                 ):
        super().__init__()
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                        ...
                                        update_sampling_layers=update_sampling_layers,
        ...
        self.avg_pool = nn.AvgPool2d((8,1), 1)
        # self.rec_embed = nn.Linear(1,25)
        self.rec_cls = None

        # sampling update
        self.update_sampling_feature = update_sampling_feature
        self.update_sampling_layers = update_sampling_layers
        self.decoder.pooler = self.pooler
        self.decoder.num_feature_levels = self.num_feature_levels
        self.decoder.avg_pool = self.avg_pool
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, H=None, W=None, rec_embed=None):
        """
        Input:
        ...
        if self.two_stage_type =='standard':
            ...
            # [bs, all hw, num_classes=2]
            enc_outputs_class_unselected = self.enc_out_class_embed(output_memory)
            # [bs, all hw, 4]
            enc_outputs_coord_unselected = self.enc_out_bbox_embed(output_memory) + output_proposals # (bs, \sum{hw}, 4)
            ...
            init_boxes = box_ops.box_cxcywh_to_xyxy(refpoint_embed.sigmoid())

            B, N, _ = init_boxes.shape
            init_boxes[:, :, 0] *= spatial_shapes[0][1]
            init_boxes[:, :, 1] *= spatial_shapes[0][0]
            init_boxes[:, :, 2] *= spatial_shapes[0][1]
            init_boxes[:, :, 3] *= spatial_shapes[0][0]
            init_boxes = init_boxes * valid_ratios[:,0].view(B, 1, 2).repeat(1,1,2)
            init_boxes = torch.clamp(init_boxes, min=0) * 8
            proposal_boxes = [Boxes(i) for i in init_boxes]
            if self.level_embed.shape[0]==5:
                roi_features = self.pooler(srcs[:-2], proposal_boxes)
            else:
                roi_features = self.pooler(srcs[:-1], proposal_boxes)
            roi_features = self.avg_pool(roi_features)
            roi_features = roi_features.reshape(B, N,  256, 25).permute(0,1,3,2)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        self.rm_detach = None
        self.text_pos_embed  = PositionalEncoding1D(self.d_model, normalize=True, scale=1)
        # self.text_embed = nn.Embedding(25, self.d_model)
        self.num_decoder_layers = num_decoder_layers
        self.update_sampling_layers = update_sampling_layers
        self.update_weight = update_weight

    def forward(self, tgt, roi_features, memory,
        ...
        text_pos_embed = self.text_pos_embed(tgt)[None, ...].repeat(tgt.shape[0], 1, 1)
        rec_cls = self.rec_cls(output[:,:,1:])

        for layer_id, layer in enumerate(self.layers):
            muti_mask = layer._get_location_mask(output.shape[2], output.device)
            q = layer.with_pos_embed(output, text_pos_embed.unsqueeze(1))
            v = torch.cat((output[:,:,:1],self.rec_proj(rec_cls.softmax(-1))),2)
            k = layer.with_pos_embed(v, text_pos_embed.unsqueeze(1))
            # VLC
            output2 = layer.attn_m(
                q.flatten(0, 1).transpose(0, 1), 
                k.flatten(0, 1).transpose(0, 1), 
                v.flatten(0, 1).transpose(0, 1),
                attn_mask=muti_mask,
            )[0].transpose(0, 1).reshape(q.shape)
            output = output + layer.dropout_m(output2)
            output = layer.norm_m(output)
            output = layer.forward_ffn_m(output)
            ...
            if self.bbox_embed is not None:
                ...
                delta_unsig = self.bbox_embed[layer_id](output[:,:,0])
                outputs_unsig = delta_unsig + reference_before_sigmoid
                new_reference_points = outputs_unsig.sigmoid()
                rec_cls = self.rec_cls(output[:,:,1:])

                intermediate_rec.append(rec_cls)
                # select # ref points
                if self.dec_layer_number is not None and layer_id != self.num_layers - 1:
                    ...
                    if nq_now != select_number:
                        class_unselected = self.class_embed[layer_id](output[:,:,0]) # nq, bs, 91
            ...
            # sampling update
            if self.pooler is not None and layer_id < self.update_sampling_layers: # 5
                new_ref_points = new_reference_points.detach()
                init_boxes = box_ops.box_cxcywh_to_xyxy(new_ref_points.sigmoid()).transpose(1, 0)
                ...
                roi_features = roi_features.reshape(B, N,  256, 25).permute(0,1,3,2)
                roi_features = roi_features.transpose(0, 1)
                recognition_query_clone = output[:, :, 1:].clone()
                recognition_query_clone = self.update_weight * recognition_query_clone + (1 - self.update_weight) * layer.dropout_s(roi_features)
                recognition_query_clone = layer.norm_s(recognition_query_clone)
                output[:,:,1:] = layer.forward_ffn_s(recognition_query_clone)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # self.rec_proj = nn.Linear(97, d_model)
        # self attention
        ...
        self.attn_m = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        ...
        # ffn_m
        self.linear1_m = nn.Linear(d_model, d_ffn)
        self.activation_m = _get_activation_fn(activation, d_model=d_ffn, batch_dim=1)
        self.dropout3_m = nn.Dropout(dropout)
        self.linear2_m = nn.Linear(d_ffn, d_model)
        self.dropout4_m = nn.Dropout(dropout)
        self.norm3_m = nn.LayerNorm(d_model)

        # ffn_s
        self.linear1_s = nn.Linear(d_model, d_ffn)
        self.activation_s = _get_activation_fn(activation, d_model=d_ffn, batch_dim=1)
        self.dropout3_s = nn.Dropout(dropout)
        self.linear2_s = nn.Linear(d_ffn, d_model)
        self.dropout4_s = nn.Dropout(dropout)
        self.norm3_s = nn.LayerNorm(d_model)
    ...
    def forward_ffn_m(self, tgt):
        tgt2 = self.linear2_m(self.dropout3_m(self.activation_m(self.linear1_m(tgt))))
        tgt = tgt + self.dropout4_m(tgt2)
        tgt = self.norm3_m(tgt)
        return tgt

    def forward_ffn_s(self, tgt):
        tgt2 = self.linear2_s(self.dropout3_s(self.activation_s(self.linear1_s(tgt))))
        tgt = tgt + self.dropout4_s(tgt2)
        tgt = self.norm3_s(tgt)
        return tgt
    ...
    def forward_sa(self,
                ...
                text_pos_embed: Optional[Tensor] = None,
        ...
        # self attention
        if self.attn_inter is not None:
            if self.decoder_sa_type == 'sa':
                q = k = self.with_pos_embed(tgt, text_pos_embed.unsqueeze(1))
                ...
                tgt_query_pos = torch.swapdims(tgt_query_pos.unsqueeze(2), 0, 2)
                q_inter = k_inter = self.with_pos_embed(tgt_inter, tgt_query_pos)
                ...
                tgt = torch.swapdims(self.norm_inter(tgt_inter), 0, 2)
    ...
    def forward_ca(self,
        ...
        tgt_reference_points = tgt_reference_points[:, :, None, :, :].repeat(1, 1, tgt.shape[2], 1, 1)
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, tgt_query_pos.unsqueeze(2)).transpose(0, 1).flatten(1,2),
                               tgt_reference_points.transpose(0, 1).contiguous().flatten(1,2),
                               memory.transpose(0, 1), memory_spatial_shapes, memory_level_start_index, memory_key_padding_mask).reshape(tgt.transpose(0, 1).shape)
    ...
    def forward(self,
        ...
        for funcname in self.module_seq:
            if funcname == 'ffn':
                ...
                tgt = self.forward_ca(tgt, tgt_query_pos, text_pos_embed, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
            elif funcname == 'sa':
                tgt = self.forward_sa(tgt, tgt_query_pos, text_pos_embed, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
            else:
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        use_detached_boxes_dec_out=use_detached_boxes_dec_out,
        update_sampling_feature=args.update_sampling_feature,
        update_sampling_layers=args.update_sampling_layers,
